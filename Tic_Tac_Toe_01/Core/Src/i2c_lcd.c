...
void lcd_init(I2C_HandleTypeDef* hi2c)
{
	...
	buf[3] = 0x0C; // Display on, Cursor off, Blink off
...
void lcd_set_mode(lcd_mode_t mode)
{
    switch (mode)
    {
        case LCD_MODE_DISPLAY:
            lcd_set_cur_blink(off, off);
            break;

        case LCD_MODE_INPUT:
            lcd_set_cur_blink(on, on);
            break;
...
void lcd_show_input(const char* line1, const char* line2, uint8_t cursor_x, uint8_t cursor_y)
{
    ...
    lcd_cur_pos(cursor_x, cursor_y);
    lcd_set_mode(LCD_MODE_INPUT);
}
